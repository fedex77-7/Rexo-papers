...
4. Open that JSON file, copy its **entire contents** — you'll paste it as the `FIREBASE_CREDENTIALS_JSON` environment variable (as one line/string) when deploying
...
6. Under **Environment**, add these variables:
   ...
   - `FIREBASE_CREDENTIALS_JSON` (the full JSON from Step 3, pasted as one string)
   - `GROQ_API_KEY`
