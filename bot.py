...
def main_menu_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🤖 Custom PDF (Chat)", callback_data="custom_pdf")],
...
def my_keys_kb(keys):
    rows = []
    for k in keys:
        left = "∞" if k.get("unlimited") else f"{k.get('entries_left', 0)} left"
        label = f"{k['key'][:6]}… ({left}, {k.get('tier', 'standard')})"
        rows.append([InlineKeyboardButton(label, callback_data=f"usekey_{k['key']}")])
    rows.append([InlineKeyboardButton("➕ Add new key", callback_data="add_key")])
...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await q.edit_message_text(text, reply_markup=contact_kb())
        ...
        context.user_data["mode"] = "question_paper"
        if not ok:
        ...
        if not ok:
            ...
            context.user_data["state"] = "awaiting_custom_desc"
            await q.edit_message_text(
                "🤖 *Custom PDF*\n\nDescribe what you want, in your own words — a certificate, "
                "notice, letter, worksheet, invitation, anything. Send it as one message.\n\n"
                "Example: \"A participation certificate for a school sports day, with a blank "
                "for the student's name and event name.\"",
                parse_mode="Markdown",
            )

    elif data == "my_keys":
        keys = db.list_known_keys(uid)
        if not keys:
            await q.edit_message_text(
                "You haven't used any keys yet.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("➕ Add a key", callback_data="add_key")],
                    [InlineKeyboardButton("⬅️ Back", callback_data="settings")],
                ]),
            )
        else:
            await q.edit_message_text("🔑 Your keys — tap one to make it active:", reply_markup=my_keys_kb(keys))

    elif data == "add_key":
        context.user_data["state"] = "awaiting_key"
        context.user_data.setdefault("mode", "question_paper")
        await q.edit_message_text("🔑 Send the new access key:")

    elif data.startswith("usekey_"):
        key = data.replace("usekey_", "", 1)
        ok, reason, key_data = db.validate_key(key)
        if not ok:
            reasons = {
            ...
            await q.edit_message_text(reasons.get(reason, "❌ That key is no longer valid.") + "\n\nChoose another:", reply_markup=my_keys_kb(db.list_known_keys(uid)))
        else:
            db.update_user(uid, active_key=key)
            left_label = "unlimited" if key_data.get("unlimited") else f"{key_data['entries_left']}"
            await q.edit_message_text(f"✅ Active key set. {left_label} paper(s) available.", reply_markup=settings_kb())
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "awaiting_key":
        ...
        db.add_known_key(uid, text)
        left_label = "Unlimited" if data_.get("unlimited") else str(data_['entries_left'])
        mode = context.user_data.get("mode", "question_paper")
        if mode == "custom":
            context.user_data["state"] = "awaiting_custom_desc"
            await update.message.reply_text(
                f"✅ Key accepted. {left_label} paper(s) available on this key.\n\n"
                "🤖 Now describe what PDF you want, in your own words (one message)."
            )
        else:
            context.user_data["state"] = "awaiting_meta"
            context.user_data["meta"] = {}
    ...
    if state == "awaiting_custom_desc":
        await update.message.reply_text("⏳ Creating your document with AI...")
        try:
            content = ai_format.generate_custom_document(text)
        except Exception as e:
            await update.message.reply_text(f"⚠️ AI generation failed: {e}\nTry again with a clearer description.")
            return

        context.user_data["custom_content"] = content

        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
            preview_path = tmp.name
        pdf_maker.build_generic_pdf(preview_path, content, watermark=True)
...
async def send_final_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    custom_content = context.user_data.get("custom_content")
    meta = context.user_data.get("meta", {})

    have_content = parsed if mode == "question_paper" else custom_content
    if not have_content or not active_key:
    ...
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
        final_path = tmp.name
    watermark = key_data.get("tier") not in ("a_plus", "first_class")
    if mode == "custom":
        pdf_maker.build_generic_pdf(final_path, custom_content, watermark=watermark)
    else:
        pdf_maker.build_pdf(final_path, parsed, meta, watermark=watermark)

    _, _, refreshed = db.validate_key(active_key)
    if refreshed and refreshed.get("unlimited"):
        left = "Unlimited"
    else:
...
async def admin_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if state == "adm_awaiting_create":
        ...
        try:
            entries = int(parts[0])
            days = int(parts[1])
            raw_tier = parts[2] if len(parts) > 2 else "standard"
            tier = db.normalize_tier(raw_tier)
            note = " ".join(parts[3:]) if len(parts) > 3 else ""
        except (IndexError, ValueError):
            await update.message.reply_text(
                "Format: <entries> <valid_days> [tier] [note]\n"
                "Tiers: standard / a_plus / first_class (first_class = unlimited papers, no watermark)\n"
                "Example: 50 30 a_plus Green Valley School"
            )
            return True
        key = db.create_key(entries, days, tier, note)
        context.user_data["state"] = None
        entries_label = "unlimited" if tier == "first_class" else entries
        await update.message.reply_text(f"✅ Key created:\n`{key}`\n{entries_label} entries, {days} days, tier={tier}", parse_mode="Markdown")
        return True
...
async def admin_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_create":
        ...
        await q.message.reply_text(
            "Send: <entries> <valid_days> [tier] [note]\n"
            "Tiers: standard (watermark) / a_plus (no watermark, 50 suggested) / first_class (no watermark, unlimited — entries number ignored)\n"
            "Example: 50 30 a_plus Green Valley School"
        )
        ...
        lines = []
        for k in keys:
            left = "∞" if k.get("unlimited") else f"{k['entries_left']}/{k['entries_total']}"
            lines.append(f"`{k['key']}` — {left} left, tier={k.get('tier')}, revoked={k.get('revoked')}")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    """Catches any unhandled exception in a handler so the bot never goes
    silent — logs it, and if possible tells the user something broke
    instead of leaving them staring at an unresponsive tap."""
    print(f"Unhandled error: {context.error}")
    try:
        if isinstance(update, Update) and update.effective_chat:
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="⚠️ Something went wrong on that action. Please try again, or use /start.",
            )
    except Exception:
        pass
